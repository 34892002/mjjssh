...
import type { AiAgentConfig, AiConnectionTestResult } from '../types/ai'

const aiStore = useAiStore()
const activeTab = ref('provider')
const configForm = ref({ baseUrl: '', apiKey: '', model: '', timeoutSeconds: 60 })
...
function syncConfigForm() {
  ...
  configForm.value.model = aiStore.config.model ?? ''
...
async function testConnection() {
  ...
  connectionTest.value = null
  try {
    connectionTest.value = await aiStore.testConnection()
  } finally {
...
<template>
  <section class="ai-settings">
    ...
    <n-tabs v-model:value="activeTab" type="line" class="ai-tabs" animated>
      <n-tab-pane name="provider" tab="提供商">
        <section class="tab-section">
          ...
          <div class="settings-card">
            ...
            <div class="settings-row">
              <div class="row-description"><strong>模型</strong><p>填写服务商提供的可用模型名称。</p></div>
              <n-input v-model:value="configForm.model" placeholder="例如 gpt-4.1-mini" class="row-control" />
            </div>
            <div class="settings-row">
          ...
          <div class="provider-actions">
            ...
            <span v-if="configured" class="configured-status">已保存配置</span>
          </div>
      ...
      <n-tab-pane name="security" tab="安全"><section class="coming-soon"><strong>安全设置即将推出</strong><p>将提供更细致的数据外发与执行权限控制。</p></section></n-tab-pane>
    </n-tabs>
  </section>
</template>
...
@media (max-width: 620px) { .ai-tabs :deep(.n-tabs-tab) { min-width: auto; padding: 7px 10px; }.settings-row { align-items: stretch; flex-direction: column; gap: 12px; }.row-control { width: 100%; }.timeout-control { width: 100%; } }
</style>
